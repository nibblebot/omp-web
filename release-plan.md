...
5. **Changelog** (LLM-assisted, deterministic structure):
   ...
   - LLM prose: one-turn SDK session (lazy `await import("@oh-my-pi/pi-coding-agent")` — the same lazy-import philosophy as `fleet/omp-check.ts`), default-role model from settings, temp project dir, `hasUI: false`; prompt = grouped commit list + instructions; output = release overview paragraph + polished bullets. Any failure (no SDK, no auth, no model, timeout, session error) → **deterministic fallback**: raw subjects, no overview. `--no-llm` forces the fallback.
