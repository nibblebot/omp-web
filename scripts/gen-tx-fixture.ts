...
function sysLine(type: string, id: string, iso: string, extra: Record<string, unknown> = {}): string {
...
const toolCall = (id: string, name: string, args: Record<string, unknown>): Record<string, unknown> => ({
  type: "toolCall",
  id,
  name,
  arguments: args,
...
function assistantLine(
  ...
  const msg: Record<string, unknown> = {
    ...
    usage: {
      ...
      cost: turn.cost,
      ...(turn.usage.reasoningTokens !== undefined ? { reasoningTokens: turn.usage.reasoningTokens } : {}),
    },
  ...
  const line = JSON.stringify({ type: "message", id: turn.entryId, parentId: null, timestamp: turn.iso, message: msg });
...
function toolResultLine(
  ...
  return JSON.stringify({
    ...
    parentId: null,
    timestamp: iso,
    message: { role: "toolResult", toolCallId, isError, content: [{ type: "text", text: content }], ...extra },
  });
}

/** Link a tool-result to its tool_calls row (fills result_chars / is_error). */
function linkResult(toolRows: ToolRow[], toolCallId: string, content: string, isError: boolean): void {
...
type UserTextStats = Omit<UserRow, "session_file" | "entry_id" | "folder" | "timestamp" | "model" | "provider">;

const user = (entryId: string, iso: string, content: string, s: SessionData, dbRow?: UserTextStats): void => {
...
const sys = (type: string, id: string, iso: string, s: SessionData, extra?: Record<string, unknown>): void => {
...
const execStart = (
  ...
  s.lines.push(
    JSON.stringify({ type: "custom", id: entryId, parentId: null, timestamp: iso, customType: "tool_execution_start", data }),
  );
...
A.lines.push(
  ...
  sessionHeader("019f3389-f804-7000-b786-e5f55c0d7181", "Alpha Session", "2026-01-01T10:00:00.000Z", "/var/home/joshua/work/alpha"),
);
sys("model_change", "mc-1", "2026-01-01T10:00:00.250Z", A, { from: "claude-sonnet-4", to: "claude-opus-4-1", model: "claude-opus-4-1", role: "primary", resolvedModelIsFallback: false });
sys("thinking_level_change", "tlc-1", "2026-01-01T10:00:00.500Z", A, { thinkingLevel: "high", configured: true, level: "high" });
user("user-1", "2026-01-01T10:00:00.750Z", "Search for bun sqlite performance and read server.ts", A, {
  chars: 52,
  words: 8,
  yelling: 0,
  profanity: 0,
...
main(
  {
    ...
    usage: { input: 300, output: 90, cacheRead: 50, cacheWrite: 10, total: 450, premium: 0, reasoningTokens: 12 },
    cost: { input: 0.0015, output: 0.0027, cacheRead: 0.00025, cacheWrite: 0.0001, total: 0.00455 },
  },
  A,
);
execStart("es-web-1", "2026-01-01T10:00:05.100Z", "call-web-1", "web_search", { query: "bun sqlite performance" }, A, "Searching bun sqlite benchmarks");
execStart("es-read-1", "2026-01-01T10:00:05.200Z", "call-read-1", "read", { path: "src/server.ts", lines: "1-40" }, A, "Reading server source");
result("res-web-1", "2026-01-01T10:00:07.250Z", "call-web-1", "Bun 1.2 benchmarks: 1.4M req/s", A, false, {
  toolName: "web_search",
  details: { timeoutSeconds: 30, wallTimeMs: 1250 },
  useless: false,
});
result("res-read-1", "2026-01-01T10:00:07.500Z", "call-read-1", "export function toAbs(...)", A);
sys("compaction", "cmp-1", "2026-01-01T10:00:08.000Z", A, { summary: "Summarized the conversation so far." });
...
sys("title_change", "tc-1", "2026-01-01T10:00:08.900Z", A, { previousTitle: "Alpha Session", title: "Alpha Session v2", source: "auto", trigger: "first_message" });
sys("credential_pin", "cp-1", "2026-01-01T10:00:08.950Z", A);
// New-accessor coverage block (types the viewer renders exhaustively).
sys("mode_change", "moc-1", "2026-01-01T10:00:08.960Z", A, { mode: "plan", data: { planFilePath: "docs/plan.md" } });
A.lines.push(
  JSON.stringify({
    type: "message",
    id: "dev-1",
    ...
    message: { role: "developer", content: [{ type: "text", text: "System: stay concise and verify claims before asserting." }] },
  }),
);
A.lines.push(
  JSON.stringify({
...
sys("custom", "gc-1", "2026-01-01T10:00:08.985Z", A, {
  ...
  data: { objective: "Make the session viewer transcript exhaustive", timeUsedSeconds: 12.5, tokensUsed: 2345 },
});
main(
  {
    entryId: "msg-3",
    iso: "2026-01-01T10:00:09.000Z",
    blocks: [toolCall("call-edit-1", "edit", { path: "src/server.ts", oldString: "a", newString: "b" })],
    duration: 312.75,
    ttft: 95,
    stopReason: "error",
...
sys("model_change", "mc-2", "2026-01-01T10:00:10.000Z", A, { from: "claude-opus-4-1", to: "claude-sonnet-4", model: "claude-sonnet-4", role: "primary", resolvedModelIsFallback: false });
sys("thinking_level_change", "tlc-2", "2026-01-01T10:00:10.100Z", A, { thinkingLevel: "low", configured: false, level: "low" });
sys("custom_message", "cusm-2", "2026-01-01T10:00:10.200Z", A, { content: "Another custom message." });
...
sys("ttsr_injection", "tti-2", "2026-01-01T10:00:11.100Z", A, { injectedRules: ["Answer in the user's language."] });
...
sys("title_change", "tc-2", "2026-01-01T10:00:11.500Z", A, { previousTitle: "Alpha Session v2", title: "Alpha Session", source: "manual", trigger: "user" });
...
sys("thinking_level_change", "tlc-3", "2026-01-01T10:00:12.000Z", A, { thinkingLevel: "high", configured: true, level: "high" });
sys("model_change", "mc-3", "2026-01-01T10:00:12.100Z", A, { from: "claude-sonnet-4", to: "claude-opus-4-1", model: "claude-opus-4-1", role: "primary", resolvedModelIsFallback: false });
sys("service_tier_change", "stc-2", "2026-01-01T10:00:12.200Z", A, { serviceTier: "standard" });
sys("custom_message", "cusm-3", "2026-01-01T10:00:12.300Z", A, { content: "One more custom message." });
  ...
  JSON.stringify({
    ...
    message: {
      ...
      usage: {
        ...
        totalTokens: 700,
        premiumRequests: 0,
        cost: { input: 0.0025, output: 0.0036, cacheRead: 0.0003, cacheWrite: 0.0002, total: 0.0066 },
      },
      duration: 900,
      stopReason: "stop",
...
B.lines.push(
  ...
  sessionHeader("019f3389-f804-7000-b786-e5f55c0d7182", "Beta Session", "2026-01-02T10:00:00.000Z", "/var/home/joshua/work/beta"),
);
...
C.lines.push(
  ...
  sessionHeader("019f3389-f804-7000-b786-e5f55c0d7183", "Gamma Session", "2026-01-03T10:00:00.000Z", "/var/home/joshua/work/gamma"),
);
...
const D = newSession(FOLDER_D, "2026-01-04T10-00-00-000Z_dddd.jsonl", `proj-d/2026-01-04T10-00-00-000Z_dddd.jsonl`);
D.msgRows.push({
...
const subFiles: Record<string, string> = {
  "SubOne.jsonl": [
    ...
    sessionHeader("019f3389-f804-7000-b786-e5f55c0d7184", null, "2026-01-01T10:00:20.000Z", "/var/home/joshua/work/alpha"),
    JSON.stringify({
      type: "session_init",
      id: "sub-si-1",
      parentId: "msg-2",
    ...
    JSON.stringify({
      ...
      message: { role: "assistant", content: [{ type: "text", text: "Subagent result: 42." }], timestamp: Date.parse("2026-01-01T10:00:21.000Z") },
    }),
  ].join("\n") + "\n",
  "__advisor.jsonl": [
    titleSlot("Advisor", "2026-01-01T10:00:22.000Z"),
    sessionHeader("019f3389-f804-7000-b786-e5f55c0d7185", null, "2026-01-01T10:00:22.000Z", "/var/home/joshua/work/alpha"),
    JSON.stringify({
      type: "message",
      id: "adv-msg-1",
      parentId: "msg-2",
      timestamp: "2026-01-01T10:00:23.000Z",
      message: { role: "assistant", content: [{ type: "text", text: "Advisory note: keep it simple." }], timestamp: Date.parse("2026-01-01T10:00:23.000Z") },
    }),
  ].join("\n") + "\n",
};
...
function expectedTools(s: SessionData, hasJsonl: boolean): Record<string, ExpectedTool> {
  ...
  for (const t of s.toolRows) {
    ...
    if (!e) {
      e = { calls: 0, errors: 0, pending: 0, totalMs: 0, avgMs: null, maxMs: null, argsChars: 0, resultChars: 0 };
...
function expectedLongest(s: SessionData): { toolName: string; toolCallId: string; durationMs: number; args: string } | null {
  let best: { toolName: string; toolCallId: string; durationMs: number; args: string } | null = null;
  let bestMs = -1;
  for (const t of s.toolRows) {
...
export const EXPECT = {
  ...
  projAStats: {
    ...
    errors: A.msgRows.filter((r) => r.stop_reason === "error").map((r) => ({
  ...
  try {
    for (const s of all) {
      for (const r of s.msgRows) {
        insMsg.run(
          r.session_file, r.entry_id, r.folder, r.model, r.provider, r.api, r.timestamp,
          r.duration, r.ttft, r.stop_reason, r.error_message, r.input_tokens, r.output_tokens,
          r.cache_read_tokens, r.cache_write_tokens, r.total_tokens, r.premium_requests,
      ...
      for (const r of s.toolRows) {
        insTool.run(
          r.session_file, r.entry_id, r.tool_call_id, r.folder, r.tool_name, r.model,
          r.provider, r.timestamp, r.agent_type, r.calls_in_turn, r.args_chars,
          r.result_chars, r.is_error,
        );
      }
      for (const r of s.userRows) {
        insUser.run(
          r.session_file, r.entry_id, r.folder, r.timestamp, r.model, r.provider,
          r.chars, r.words, r.yelling, r.profanity, r.anguish, r.negation, r.repetition, r.blame,
        );
      }
    }
