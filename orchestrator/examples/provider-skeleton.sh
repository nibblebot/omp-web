#!/usr/bin/env bash
# provider-skeleton.sh — provision-hook skeleton (orchestrator/examples).
#
# Wired up as config.spawnHook in ~/.omp/orchestrator/config.json:
#   { "spawnHook": "/path/to/omp-web/orchestrator/examples/provider-skeleton.sh" }
# (or OMP_ORCHESTRATOR_SPAWN_HOOK). The orchestrator runs it via `sh -c`
...
# non-empty stdout line — that is the enroll handshake: the orchestrator
...
#     from the orchestrator. It never learns the orchestrator's address,
#     state file, or credentials.
#   * The token is minted HERE, passed to the daemon at boot, and returned
#     to the orchestrator in the JSON. It gates ONLY this daemon — each
...
#     the orchestrator's internals. ompd enforces it: a non-loopback bind
#     without a token is a startup hard error.
#   * The orchestrator stores the token in ~/.omp/orchestrator/state.json
...
# Default demo: run ompd directly on this host in the background and parse
# its OMPD| listening line. For a remote/container provider, replace this
# block with your ssh/docker/cloud invocation (see docker-ompd.sh for the
# published-port pattern) and keep the JSON handshake below unchanged.
#
# The daemon MUST outlive this script: the orchestrator dials it right
# after the hook exits. The local demo relies on ompd's own idle exit
...
LOG="$(mktemp)"
nohup ompd --cwd "$CWD" --port 0 --host 127.0.0.1 \
	--token "$TOKEN" --name "$NAME" >"$LOG" 2>&1 &

URL=""
for _ in $(seq 1 100); do # up to ~50s; the orchestrator allows 60s total
	LINE="$(grep '^OMPD|' "$LOG" | head -n1 || true)"
	if [ -n "$LINE" ]; then
		URL="$(printf '%s' "${LINE#OMPD|}" | sed -n 's/.*"url":"\([^"]*\)".*/\1/p')"
		[ -n "$URL" ] && break
	fi
	sleep 0.5
done
