...
import { attachSession, listProjects, sendAddExistingWorktree, sendCreateWorktree, setState, state } from "../state";
 ...
 * project. Two tabs — Create new (git worktree add under the fleet
 * workspace dir; advanced section overrides the base ref or attaches an
 * existing branch) and Add existing (discovered-but-unregistered worktrees
			...
			<Show when={!busy() && project() !== null}>
				...
				<Show when={tab() === "create"}>
					...
					>
						...
						<details class="worktree-advanced">
							<summary>Advanced</summary>
							<label class="daemon-detail-label" for="worktree-base">
								base ref (default: origin/HEAD → local default → HEAD)
							</label>
							<input
								id="worktree-base"
								class="picker-filter worktree-base"
								placeholder="main, origin/main, <sha>…"
								value={baseRef()}
								onInput={e => setBaseRef(e.currentTarget.value)}
								spellcheck={false}
							/>
							<label class="daemon-detail-label" for="worktree-branch">
								existing branch (attach instead of creating; not checked out elsewhere)
							</label>
							<input
								id="worktree-branch"
								class="picker-filter worktree-branch"
