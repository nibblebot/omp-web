...
export function TxBrowser() {
  ...
  const [health] = createResource(() => syncTick(), () => api.health());
  ...
  return (
    <div class="tx-app">
      <aside class="tx-sidebar">
        <SessionList selectedFile={file()} onOpen={navigate} health={healthSafe} syncTick={syncTick} onSynced={bumpSync} />
      </aside>
      <main class="tx-main">
...
function HealthBanner(props: { health: () => Health | undefined; onSynced: () => void }) {
  ...
  return (
    <Show when={props.health()} fallback={null}>
      {(h) =>
        h().statsDb !== "ok" ? (
          <div classList={{ "health-banner": true, warn: h().statsDb === "missing", error: h().statsDb === "error" }}>
            <Show
            ...
            <button type="button" class="btn btn-small" disabled={syncing()} onClick={() => void runSync()}>
