...
import { buildDayRows, filterTranscriptEntries, progressTotal } from "./components/transcript/index";
...
describe("pairToolCalls", () => {
  ...
  test("execution-start may precede the assistant message within the loaded range", () => {
    ...
    const page2 = [assistantWithToolCall("p2-b", "call-b", "bash"), toolResult("res-b", "call-b", "ok")];
  ...
  test("pairing memo recomputes once per entries change (never per row)", () => {
    ...
    const page1 = [execStart("es-a", "call-a", "read"), assistantWithToolCall("p1-2", "call-a", "read")];
    ...
    expect(root.pairing().calls.get("call-a")).toMatchObject({ hasResult: true, startedAt: CALL_START_MS });
...
describe("buildDayRows", () => {
  ...
  test("no separator between same-day consecutive entries", () => {
    const rows = buildDayRows([msgAt("a", noon(2026, 2, 1)), msgAt("b", noon(2026, 2, 1) + 60_000)]);
  ...
  test("undated leading rows never produce a spurious separator", () => {
    ...
    const rows = buildDayRows([undated, msgAt("h", noon(2026, 2, 1)), msgAt("d2", noon(2026, 2, 2))]);
    expect(rows.map((r) => r.kind)).toEqual(["entry", "entry", "day", "entry"]);
  });

  test("null timestamps never separate and never reset the running day", () => {
    const undated: RawEntry = { type: "label", id: "undated", label: "?" };
    const rows = buildDayRows([msgAt("d1", noon(2026, 2, 1)), undated, msgAt("d2", noon(2026, 2, 2))]);
