 ...
 */
export function toolExecutionStartOf(e: RawEntry): { toolCallId: string; startedAtMs: number } | null {
...
export function contentBlocks(e: RawEntry): ContentBlock[] {
  ...
  if (Array.isArray(c)) {
    return c.filter((b): b is ContentBlock => typeof b === "object" && b !== null && !Array.isArray(b));
  }
...
export function systemDetail(e: RawEntry): string {
  ...
  const keys = ["model", "to", "from", "name", "title", "level", "service", "id", "cwd", "summary", "text", "message", "reason"];
  const parts: string[] = [];
  for (const k of keys) {
    const v = e[k];
...
export function modeChangeOf(e: RawEntry): { mode: string | null; data: Record<string, unknown> | null } | null {
...
export function modelChangeOf(e: RawEntry): { model: string | null; role: string | null; resolvedModelIsFallback: boolean } | null {
...
export function thinkingLevelChangeOf(e: RawEntry): { thinkingLevel: string | null; configured: unknown } | null {
  if (e.type !== "thinking_level_change") return null;
  return { thinkingLevel: typeof e.thinkingLevel === "string" ? e.thinkingLevel : null, configured: e.configured };
}
...
export function titleChangeOf(e: RawEntry): { previousTitle: string | null; title: string | null; source: string | null; trigger: string | null } | null {
...
export function customEntryOf(e: RawEntry): { customType: string; data: Record<string, unknown> | null } | null {
...
export interface AssistantMeta {
  ...
  usage: { input: number | null; output: number | null; cacheRead: number | null; cacheWrite: number | null; reasoningTokens: number | null; costTotal: number | null } | null;
}
...
export function assistantMetaOf(e: RawEntry): AssistantMeta | null {
  ...
  return {
    ...
    contextSnapshot: ctx === null ? null : { promptTokens: num(ctx.promptTokens), nonMessageTokens: num(ctx.nonMessageTokens) },
    usage:
      usage === null
        ? null
        : {
...
export function shortSummary(e: RawEntry, maxLen = 60): string {
  ...
  if (t === "assistant" || t === "developer") {
    s = contentBlocks(e)
      .map((b) => (b.type === "text" ? str(b.text) : b.type === "toolCall" ? `[${str(b.name) || "tool"}]` : ""))
      .filter((p) => p !== "")
      .join(" ");
  } else if (t === "user") {
    s = userText(e);
