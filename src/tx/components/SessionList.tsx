    ...
    <div class="sidebar-inner">
      <header class="tx-sidebar-header">
        ...
        <div class="header-right">
          <span class="count">{sessionsRes.loading && sessions().length === 0 ? "…" : sessions().length}</span>
        </div>
      </header>
        ...
        <Show
          ...
          fallback={
            <Show
              ...
              fallback={
                <Show when={sessions().length > 0} fallback={<div class="list-hint">No sessions match.</div>}>
                  <For each={sessions()}>
                    {(s) => <SessionRow session={s} selected={s.file === props.selectedFile} onOpen={props.onOpen} />}
                  </For>
                </Show>
              }
            >
...
function SessionRow(props: { session: SessionSummary; selected: boolean; onOpen: (file: string) => void }) {
  const s = () => props.session;
  return (
    <button
      type="button"
      classList={{ "session-row": true, selected: props.selected, unsynced: !s().synced, missing: !s().onDisk }}
    ...
    >
      ...
      <div class="row-metrics">
        <span>{formatCompact(s().turns)} {s().turns === 1 ? "turn" : "turns"}</span>
        <span class="dot">·</span>
