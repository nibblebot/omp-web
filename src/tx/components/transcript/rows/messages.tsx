...
export function AssistantMsg(props: { entry: RawEntry; pairing: PairingMaps }) {
  ...
  const rawUsage = typeof m.usage === "object" && m.usage !== null && !Array.isArray(m.usage) ? (m.usage as Record<string, unknown>) : null;
  ...
  const ts = entryTs(e);
  const error = stopReason === "error" || errorMessage !== "" || errorId !== null || errorStatus != null;
        ...
        <ToggleDetails entry={e} purpose="usage" class="usage-details" summary="usage">
          <Show when={usage !== null}>
            <div class="usage-grid">
              <span class="usage-cell"><b>input</b> {formatCompact(usage?.input)}</span>
              <span class="usage-cell"><b>output</b> {formatCompact(usage?.output)}</span>
              <span class="usage-cell"><b>cacheRead</b> {formatCompact(usage?.cacheRead)}</span>
              <span class="usage-cell"><b>cacheWrite</b> {formatCompact(usage?.cacheWrite)}</span>
              <Show when={usage?.reasoningTokens !== null}>
                <span class="usage-cell"><b>reasoning</b> {formatCompact(usage?.reasoningTokens)}</span>
              </Show>
              <Show when={usage?.costTotal !== null}>
                <span class="usage-cell"><b>cost</b> {formatCost(usage?.costTotal)}</span>
              </Show>
            </div>
          </Show>
          <Show when={ctx !== null}>
            <div class="usage-grid">
              <span class="usage-cell"><b>ctx prompt</b> {formatCompact(ctx?.promptTokens)}</span>
              <span class="usage-cell"><b>ctx non-message</b> {formatCompact(ctx?.nonMessageTokens)}</span>
            </div>
          </Show>
...
function Block(props: { block: ContentBlock; pairing: PairingMaps; entry: RawEntry; idx: number }) {
  ...
  if (b.type === "thinking") {
    ...
    return text !== "" ? (
      <ToggleDetails entry={props.entry} purpose={`thinking:${props.idx}`} class="thinking" summary="thinking">
        <Markdown src={text} />
      </ToggleDetails>
    ) : null;
  }
  if (b.type === "toolCall") return <ToolCallCard block={b} pairing={props.pairing} entry={props.entry} idx={props.idx} />;
  return (
...
function ToolCallCard(props: { block: ContentBlock; pairing: PairingMaps; entry: RawEntry; idx: number }) {
  ...
  return (
    <div class="tool-call">
      ...
      <ToggleDetails entry={props.entry} purpose={`tc-args:${props.idx}`} class="tc-args" summary="arguments">
        <pre>{args}</pre>
      </ToggleDetails>
...
function ToolResultSub(props: { entry: RawEntry; startedAt: number | null }) {
  ...
  const shown = text.length > MAX_RESULT_CHARS ? `${text.slice(0, MAX_RESULT_CHARS)}… (truncated)` : text;
  const details = meta?.details ?? null;
  const detailChips = details === null ? [] : Object.entries(details).filter(([, v]) => scalar(v) !== null);
  return (
      ...
      <Show when={detailChips.length > 0}>
        <div class="chip-row">
          <For each={detailChips}>
            {([k, v]) => <span class="mini-chip">{k}: {scalar(v)}</span>}
          </For>
        </div>
      </Show>
...
export function FileMentionMsg(props: { entry: RawEntry }) {
  ...
  const files = (Array.isArray(m.files) ? m.files : []).filter(
    (f): f is { path: string; content?: unknown } =>
      typeof f === "object" && f !== null && !Array.isArray(f) && "path" in f && typeof f.path === "string",
  );
  const row = useRowCollapse(e);
  return (
    <div classList={{ "tx-msg": true, "file-mention": true, collapsed: row.collapsed() }}>
      ...
      <For each={files}>
        {(f, i) => (
          <ToggleDetails entry={e} purpose={`file-attach:${i()}`} class="file-attach" summary={f.path}>
