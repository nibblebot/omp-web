      ...
      <Show when={s !== null}>
        <>
          <div class="sys-summary">
            ...
            <span class="tx-badge">{(s?.tools.length ?? 0) === 1 ? "1 tool" : `${s?.tools.length ?? 0} tools`}</span>
          ...
          <ToggleDetails entry={props.entry} purpose="big-payload:sysprompt" class="big-payload" summary={`system prompt · ${s?.systemPrompt.length ?? 0} chars`}>
          ...
          <Show when={s?.task !== null && s?.task !== ""}>
            <ToggleDetails entry={props.entry} purpose="big-payload:task" class="big-payload" summary={`task · ${s?.task?.length ?? 0} chars`}>
              <pre>{s?.task}</pre>
            </ToggleDetails>
          </Show>
          <Show when={s?.outputSchema !== null}>
            <ToggleDetails entry={props.entry} purpose="big-payload:schema" class="big-payload" summary="output schema">
...
export function CompactionRow(props: { entry: RawEntry }) {
  ...
  return (
    <SysCard entry={props.entry} label="compaction">
      <Show when={s !== null && s !== ""}>
        <ToggleDetails entry={props.entry} purpose="big-payload:summary" class="big-payload" summary={`summary · ${s?.length ?? 0} chars`}>
          <pre>{s}</pre>
        </ToggleDetails>
      </Show>
    </SysCard>
          ...
          <Show when={c?.details !== null && (c?.details?.notes.length ?? 0) > 0}>
            <ul class="note-list">
              <For each={c?.details?.notes ?? []}>
                {(n) => (
                  <li classList={{ "note-item": true, [`note-${NOTE_SEV_CLASS[n.severity] ?? "muted"}`]: true }}>
...
export function CustomRow(props: { entry: RawEntry }) {
  ...
  if (c.customType === "goal-completed") return <GoalCompletedRow entry={props.entry} data={c.data} />;
  return (
    <SysCard entry={props.entry} label={c.customType !== "" ? `custom · ${c.customType}` : "custom"}>
      <Show when={c.data !== null}>
        <ToggleDetails entry={props.entry} purpose="big-payload:data" class="big-payload" summary="data">
