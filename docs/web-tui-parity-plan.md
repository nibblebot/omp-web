...
- [x] Phase 0 — Protocol generalization & state mirror (2026-07-30)
  - Deviation: `setSubagentSubscription("progress")` is issued in `connect()` on every (re)open (idempotent, per RpcClient) — moved here from Phase 4 step 8 so the server's subagent forwards actually fire; Phase 4 only consumes the frames. → **Superseded 2026-08-05 (SDK conversion)**: the method is removed entirely; lifecycle/progress frames always relay at progress level in-process.
...
- [x] Phase 3 — Status bar parity (2026-07-30)
  ...
  - Gap noted: `RpcSessionState` exposes no `autoRetryEnabled`, so the auto-retry checkbox is fire-and-forget (defaults to checked, not read back). → **Resolved 2026-08-05 (SDK conversion)**: `WebSessionState` carries `autoRetryEnabled`; the checkbox reads back real state.
...
- [x] SDK conversion (2026-08-05) — the JSON-RPC child process is gone; the server hosts sessions in-process via `createAgentSession`. Transport is now browser ⇄ WebSocket ⇄ Bun in-process SDK. What this changed:
  - **Ask tool works**: extension UI dialogs are now reachable — the server implements `ExtensionUIContext` in-process and relays dialogs as `ui_request`/`ui_response` frames (supersedes Phase 6 step 3's "NOT reachable" gap).
  - **`autoRetryEnabled` is real**: the state mirror carries it and the settings checkbox reads back actual state (closes the Phase 3 gap).
  - **Subagent steer/abort available**: `subagentSteer`/`subagentAbort` relay rows (Phase 4 step 8 was read-only).
  - **Multi-session**: the server multiplexes live sessions; sockets attach/create/close via the Sessions modal.
  - **`setSubagentSubscription` removed**: it was an RPC bandwidth gate, meaningless in-process — lifecycle/progress frames always relay at progress level (supersedes the Phase 0 deviation).
  - **Identifier renames** (local copies in `src/protocol.ts`): `RpcSessionState`→`WebSessionState`, `RpcAvailableSlashCommand`→`AvailableSlashCommand`, `RpcSubagentMessagesResult`→`SubagentMessagesResult`, `RpcMethodName`→`WebMethodName`.

## Context

`omp-web` is a Solid.js web UI over `@oh-my-pi/pi-coding-agent` — originally its JSON-RPC mode (`RpcClient` over stdio, bridged to the browser by a Bun WebSocket server); **since the 2026-08-05 SDK conversion the agent runs in-process via `createAgentSession`** (see Progress → SDK conversion; the RPC-era ground facts below are kept as history). Today it supports only: prompt/abort/new-session, text+thinking streaming with a reveal queue, a generic `<details>` tool card, and a minimal status bar. The TUI (same package, interactive mode) offers ~70 slash commands, steer/follow-up/queue while streaming, model & thinking pickers, a rich status line (context %, cost, tokens, session name), per-tool renderers (bash, diff, read, todo, task), image paste, `!` bang-shell, session resume/branch/tree, compaction summaries, and an agent hub.
...
3. **Extension UI dialogs**: NOT reachable without upstream changes (`RpcClient` exposes no public `extension_ui_request` subscribe/respond). Web-only stance: leave unimplemented; extensions that block on a dialog will hang until their timeout. If this becomes painful, the upstream patch is ~20 lines (`onExtensionUIRequest(listener)` + `respondToExtensionUI(response)` on `RpcClient`) — pre-decided fallback, not part of this plan. → **Resolved 2026-08-05 (SDK conversion)**: the server implements `ExtensionUIContext` in-process and relays dialogs as `ui_request`/`ui_response` frames; the `ask` tool works.
