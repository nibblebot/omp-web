...
- **Single installable bundle** — `bun run build` → `dist-bundle/cli.js`, installed via `scripts/install-omp-web.ts`; the UI is embedded (`server/embedded-dist.ts`) so the daemon and fleet edge serve it from any cwd.
...
- [ ] **#75 — omp-fleet compiled binary.** "Single binary self-hosted" is half true while the differentiator itself needs a checkout + Bun runtime. Add `build:omp-fleet` (bun compile over `fleet/`, ~6 small modules) → a compiled `omp-fleet` binary, plus a systemd unit and container example.
