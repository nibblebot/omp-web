...
- `registry.ts` — JSON persistence (atomic tmp+rename on every mutation), monotonic `dN` id allocation floored above the max id found on disk, boot-time status reconciliation. Roster serialization (`toRosterEntry`) structurally strips token/endpoint/template before anything reaches a browser.
...
- `server.ts` + `cli.ts` — loopback control API and the `omp-fleet` CLI; `daemons-aggregator.ts` — the aggregated daemons panel feed.
