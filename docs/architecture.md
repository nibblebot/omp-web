...
- `session-title.ts` — read-only session-title probe for the roster's `sessionTitle` field: bounded 8KB head read of `lastSessionFile` (256B title slot preferred, session-header title fallback), LRU-cached by `(path, mtimeMs, size)`; called from the supervisor's git-state poll, never for remote entries, registry updated only on change.
