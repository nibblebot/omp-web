...
- **Project/worktree onboarding (additive):** new `ClientCommand` variants `add_project` (`{path, start?, template?, labels?}`), `remove_project` (`{projectId}`), `create_worktree` (`{projectId, name, baseRef?, existingBranch?, start?}`), `add_worktree` (`{projectId, worktreePath, start?}`), `delete_worktree` (`{daemonId, deleteBranch?}`) and `worktree_delete_info` (`{daemonId}`) — the edge answers them in-process (shared registry/supervisor/worktrees module) and its fail-closed command allowlist grew with them; the parallel `/ctl` routes serve the CLI/API. New fleet-scoped frames: `registered_projects` (broadcast; lets project groups with zero daemons render) and unicast `worktree_delete_info` (owned/dirty counts/branch merge+push evidence for the delete confirmation — never tokens/endpoints). `DaemonEntry` gains optional `projectId` and `managed` fields (absent for remote entries and older edges).
