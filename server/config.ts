...
export interface OmpdConfig {
	...
	cwd: string;
	/** Listen port; 0 = ephemeral (the real port is reported in the OMPD| listening line). */
	port: number;
	/** Bind address; anything non-loopback requires --token. */
	host: string;
	/** Reported verbatim as the "advertise" field of the OMPD| listening line; does not affect the bind. */
	advertise?: string;
	...
	name: string;
	/** Selector labels for orchestrator fan-out (R9). */
	labels: string[];
	/**
	 * Internal test hook (OMPD_TEST_READY_DELAY_MS): hold the readiness gate
...
/**
 * Parse flags + env into the ompd config. Throws with a human-readable
 * message on invalid input; the caller prints it to stderr and exits 1.
 */
export function parseConfig(argv: string[]): OmpdConfig {
	...
	const cwd = flag("cwd") ?? Bun.env.OMPD_CWD ?? process.cwd();

	const portRaw = flag("port") ?? Bun.env.OMPD_PORT ?? "4721";
	const port = Number(portRaw);
	if (!Number.isInteger(port) || port < 0 || port > 65535) {
		throw new Error(`invalid port "${portRaw}" (0-65535; 0 = ephemeral)`);
	}

	const host = flag("host") ?? Bun.env.OMPD_HOST ?? "127.0.0.1";

	const idleTimeoutMs = parseDuration(flag("idle-timeout") ?? Bun.env.OMPD_IDLE_TIMEOUT ?? "30m");
	...
	return {
		...
		port,
		host,
		advertise: flag("advertise") ?? Bun.env.OMPD_ADVERTISE,
		token: flag("token") ?? Bun.env.OMPD_TOKEN,
		resume: flag("resume") ?? Bun.env.OMPD_RESUME,
		idleTimeoutMs,
		name: flag("name") ?? Bun.env.OMPD_NAME ?? path.basename(cwd),
		labels,
		readyDeferMs: Math.max(0, Number(Bun.env.OMPD_TEST_READY_DELAY_MS ?? 0) || 0),
		collabMaxGuests: Number(Bun.env.OMPD_COLLAB_MAX_GUESTS ?? 64),
		collabHostname: Bun.env.OMPD_COLLAB_HOSTNAME,
		collabUrl: Bun.env.OMPD_COLLAB_URL,
	};
}
