...
describe("stats.db degradation", () => {
  ...
  test("valid stats.db → enrichment and tools still work (no regression)", async () => {
    ...
    db.exec(
      "CREATE TABLE tool_calls (tool_name TEXT, is_error INTEGER, session_file TEXT)",
    );
    ...
    db.query("INSERT INTO tool_calls (tool_name, is_error, session_file) VALUES ('edit', 0, ?)").run(abs);
    ...
    const toolsBody = (await tools.json()) as { tools: Array<{ name: string; calls: number; errors: number; sessions: number }> };
