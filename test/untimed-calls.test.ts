...
describe("untimed tool calls (no tool_execution_start marker)", () => {
    ...
    const lines = [
      ...
      JSON.stringify({
        ...
        message: {
          role: "assistant",
          timestamp: 10_000,
          content: [{ type: "toolCall", id: "call-untimed", name: "read", arguments: { path: "a" } }],
        },
      }),
      JSON.stringify({
        type: "message",
      ...
      JSON.stringify({
        ...
        message: {
          role: "assistant",
          timestamp: 30_000,
          content: [{ type: "toolCall", id: "call-pending", name: "edit", arguments: { path: "c" } }],
        },
      }),
    ].join("\n");
  ...
  const getStats = async (): Promise<StatsLike> => {
    ...
    const res = await app.handleFetch(new Request(`http://localhost${path}`), new URL(`http://localhost${path}`));
